...
# allikas : https://github.com/murumart/greg-rpg/blob/main/autoload/scr_sound.gd
...
func fade_out_song_player(player: Node, fade_speed := 1.0, options := {}) -> void:
	if not (is_instance_valid(player) and player.playing): return
	player = player as AudioStreamPlayer

	var fade_out_ease_type: Variant = options.get("fade_out_ease_type", Tween.EASE_IN)
	var trans_type: Variant = options.get("trans_type", Tween.TRANS_QUAD)
...
func _on_fadeout_tween_step_finished(player: Node, options := {}) -> void:
	# _int_stupid is provided by the tween.step_finished signal, I don't need it for anything
	var do_save_audio_position: bool = options.get("save_audio_position", true)

	if not (is_instance_valid(player) and player.playing): return
	player = player as AudioStreamPlayer

	if do_save_audio_position:
		save_audio_position(player, old_song)
	player.stop()
	player.queue_free()
...
func save_audio_position(player: AudioStreamPlayer, song: Dictionary) -> void:
...
class SongsList:
	...
	static var songs := {
		...
		&"tesla": {
			&"stream" : preload("res://music/tesla.ogg")
		},
		&"bad_end": {
